...
export function StoryStageOverlay({
  ...
  return (
    <div
      className={`absolute inset-0 z-20 grid w-full items-center overflow-hidden transition-opacity duration-500 ease-in-out
        ${isActive ? "pointer-events-auto opacity-100" : "pointer-events-none opacity-0"}`}
    >
      <div className="mx-auto relative">
        <div className="mx-8 md:mx-20 lg:ml-54 lg:mr-10 xl:ml-60 xl:mr-10 grid h-[min(84vh,58rem)] min-h-0 min-w-0 grid-rows-[auto_minmax(0,1fr)_auto] gap-[clamp(4rem,3vh,2rem)]">
          {stageCount > 0 && (
            <StoryStageHeader
          ...
          {storyStages.map((stageEntry, index) => (
            <section
              ...
              className="flex flex-row inset-0 items-center justify-between h-full transition-all duration-900 ease-in-out"
              key={stageEntry.key}
